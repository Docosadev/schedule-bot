...
import { getGuildSettings, getVoteBreakdown, getVotesForPoll } from "./db.js";
...
export async function buildPollEmbed(poll: PollWithOptions): Promise<EmbedBuilder> {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(personal
      ? "候補ごとのメッセージにリアクションしてねー！予定が変わったら、押しなおせばオッケーだぞー！"
      : "候補ごとのメッセージにリアクションしてください。予定が変わった場合はリアクションを押し直せます。")
    .addFields(
      { name: "締切", value: formatDeadline(poll.deadline, poll.timezone), inline: true },
...
export function buildPollHeaderMessage(poll: PollWithOptions, notifyMention: string, personal = false): string {
  return [
    `# **${poll.title} 日程調整**`,
    ...
    "",
    notifyMention,
    personal ? "みなのもの～！参加できる候補日にリアクションしてねー！" : "参加できる候補日にリアクションしてください。",
    formatVoteDefinitions(true),
    personal ? "予定が変わったら、リアクションを押しなおせばオッケーだぞー！" : "予定が変わった場合はリアクションを押し直せます。"
  ].join("\n");
}
...
export function buildVotedMembersMessage(names: string[], personal = false): string {
...
export async function buildResultMessage(poll: PollWithOptions, leadingMention?: string): Promise<string> {
  ...
  const winnerLines =
    winners.length > 0
      ...
      : [personal ? "投票はゼロだったぞよ！次回に期待だねー！" : "投票はありませんでした。"];

  return [
    mentions,
    `## ${poll.title} 日程調整結果`,
    `ID: ${poll.id}`,
    personal ? "投票しゅーりょー！みなのもの、ご協力ありがとー！" : "投票を締め切りました。ご協力ありがとうございました。",
    "",
    "実施候補日",
    ...winnerLines,
    "",
    personal ? "詳しい投票状況は添付画像をチェック！結果に目玉をエレキネットだぞー！" : "詳しい投票状況は添付画像をご確認ください。"
